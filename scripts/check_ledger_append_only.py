...
one record per key and a side that disagrees with ITSELF reads as clean: base
`[X]` against head `[X, A('no demonstrated edge'), A('+2.1% ROI, significant')]`
is a clean append to a comparison that keeps one record per key, and the run
after it can drop either copy with the key still present and the count held up
by the survivor. So `contradictions()` runs over BOTH sides *before* the
comparison: a ledger may not hold two records under one key that disagree about
what was found. That is not an append, it is a contradiction, and whichever
copy survives the next run is a choice nobody recorded.
...
literal — the same span written `[2023, 2024]` and `[2024, 2023]` is two keys,
...
so it cannot import `football_betting_lab`; the correction arithmetic below is
deliberately duplicated from `experiment_ledger.py` rather than imported, and
`test_the_scripts_arithmetic_matches_the_package` holds the two copies against
each other — the test suite may import the package even though this script may
not.
...
def read_ledger(path: Path, side: str) -> list[dict]:
    ...
    valid JSON. What these checks buy is the message: without them a malformed
    ledger crashes `compare()` with a `KeyError` or a `TypeError` about a dict,
    out of a step whose job is to say what happened to a hypothesis.
    """
    if not path.is_file():
    ...
    for index, entry in enumerate(entries):
        ...
        for season in seasons:
            # A bool season keys *equal* to the int: ('s', 'n', (True,)) ==
            # ('s', 'n', (1,)), so `key()` cannot tell them apart and a season
            # rewritten from 1 to `true` would pass `compare()` as the very
            # same hypothesis. `isinstance(True, int)` is True, so the bool
            # has to be named.
...
def contradictions(
    ...
    Runs over both sides and before the base/head comparison, because that
    ...
    is the same object `compare()` uses as `base_by_key`, so the two cannot
    disagree about which record is the base's.
    """
    problems: list[str] = []
...
def compare(base: list[dict], head: list[dict]) -> tuple[list[str], int]:
    ...
    # The head side keeps every entry sharing a key, so a copy that agrees
    # with its neighbours and disagrees with the base is still named.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if base is None:
        # No base is not no check: a first commit is exactly where a
        # contradictory pair would be planted so that every later run inherits
        # it as history.
